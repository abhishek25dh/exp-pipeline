...
OUTPUT_MD = ROOT / "LAYOUT_9_PHRASE_RULES_AUDIT.md"
...
def audit_scene(scene_num: int, prompt_path: Path) -> SceneAudit:
    ...
    # Pair checks: img_moment_N then txt_moment_N must be consecutive in script.
    img_re = re.compile(r"^img_moment_(\d+)$")
    txt_re = re.compile(r"^txt_moment_(\d+)$")
        ...
        if img.phrase_clean and txt.phrase_clean:
            ...
            for s in starts_img:
                ...
                if tuple(script_clean_words[next_idx : next_idx + len(txt.phrase_clean)]) == txt.phrase_clean:
                    ...
                    break
        pair_checks.append((f"moment_{step}", ok, reason))
...
def render_report(scene_audits: list[SceneAudit]) -> str:
    ...
    lines.append("# Layout 9 Phrase Rules Compliance (Proof Report)")
    lines.append("")
    ...
    lines.append("- Layout 9: `LAYOUT_PHRASE_RULES.md` (All Other Layouts section)")
    lines.append("")
    ...
    lines.append("## Scenes Using Layout 9")
    lines.append("")
    if not scene_audits:
    ...
    for sa in scene_audits:
        ...
        lines.append(f"- Rule 4 (consecutive fragments per moment): `{'PASS' if rule4_ok else 'FAIL'}`")
        lines.append("- Rule 5 (tiling coverage): `N/A` (layout not defined as tiling)")
        lines.append("")

        if sa.pair_checks:
            lines.append("### Pair Check (Image + Text Consecutive Fragments)")
            lines.append("")
            lines.append("| moment | consecutive | proof |")
            lines.append("|---|---|---|")
            for moment, ok, proof in sa.pair_checks:
                lines.append(f"| {moment} | {'YES' if ok else 'NO'} | {md_escape(proof)} |")
            lines.append("")
