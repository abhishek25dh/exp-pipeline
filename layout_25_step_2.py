"""
Layout 25 Step 2 - AI Image Descriptions for Left Images
=========================================================
Host image (host_looking_left.png) is a static local asset -- no AI needed.
Only the active left-side images get AI-generated visual descriptions.
"""
...
def fallback_description(phrase, img_id):
    clean = phrase.strip(".,!? ")
    return {
        "style": "2D colorful cartoon graphic",
        "description": (
...
def main():
    ...
    step1_path = Path("assets/tmp") / f"{scene_id}_layout_25_step_1_tmp.json"
    ...
    if not images:
        # No left images (host only) -- pass through
        out_path = Path("assets/tmp") / f"{scene_id}_layout_25_step_2_tmp.json"
        out_path.write_text(json.dumps(step1, indent=2), encoding="utf-8")
        print(f"No left images. Step 2 complete -> {out_path}")
        return 0
    ...
    for img in images:
        ...
        if not img_desc:
            ...
            desc     = fallback_description(img["phrase"], iid)
            img_desc = desc["description"]
        enriched.append({**img, "img_description": img_desc, "img_style": desc.get("style", "2D colorful cartoon graphic")})
        print(f"  {iid}: {img_desc[:80]}...")

    out = {**step1, "images": enriched}
    out_path = Path("assets/tmp") / f"{scene_id}_layout_25_step_2_tmp.json"
