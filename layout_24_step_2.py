"""
Layout 24 Step 2 - AI Image Descriptions
=========================================
Generates concrete visual descriptions for the active images in layout 24.
"""
...
def main():
    ...
    step1_path = Path("assets/tmp") / f"{scene_id}_layout_24_step_1_tmp.json"
    ...
    if not images:
        # No images — pass through
        out = {**step1, "images": []}
        out_path = Path("assets/tmp") / f"{scene_id}_layout_24_step_2_tmp.json"
        out_path.write_text(json.dumps(out, indent=2), encoding="utf-8")
        print(f"No images (text-only). Step 2 complete -> {out_path}")
        return 0
