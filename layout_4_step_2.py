"""
Layout 4 Step 2 — AI Selector + Deterministic Image Spans + AI Descriptions
=============================================================================
Layout 4: Host character + text anchor (P1) + scattered images (P2+).

AI role (single call for selection):
  1. Pick ONE anchor candidate ID from the pre-validated list.
  2. Pick text_type (text_highlighted / text_red / text_black).
  3. Pick host_emotion (explaining / happy / sad / angry).
  4. Pick host_size (full / half_top / half_bottom).

Deterministic role:
  - Snap anchor to sentence boundary (Rule 6).
  - Split remaining tokens into N image spans (no gaps, no overlaps).
...
HOST_EMOTIONS = ["explaining", "happy", "sad", "angry"]
HOST_SIZES = ["full", "half_top", "half_bottom"]

SELECTOR_SYSTEM_PROMPT = """You are choosing directorial elements for a visual explainer scene.

Given pre-validated candidates, return ONLY valid JSON:
{"candidate_id": "C3", "text_type": "text_red", "host_emotion": "explaining", "host_size": "half_bottom"}
...
- host_emotion: one of explaining/happy/sad/angry.
- host_size: one of full/half_top/half_bottom.
...
def main():
    ...
    step1_path = Path("assets/tmp") / f"{scene_id}_layout_4_step_1_tmp.json"
    ...
    # --- AI: select anchor + host config ---
    print("  Calling AI for anchor + host selection...")
    sel_raw = _api_call(
    ...
    if sel_raw:
        try:
            ...
            if tt in TEXT_TYPES:
                text_type = tt
            he = sel.get("host_emotion", "")
            if he in HOST_EMOTIONS:
                host_emotion = he
            hs = sel.get("host_size", "")
            if hs in HOST_SIZES:
                host_size = hs
        except Exception:
            pass
    ...
    print(f"  Host: emotion={host_emotion}  size={host_size}")
    ...
    # --- P0 PRIORITY: host gets the first token of the first image span ---
    # Host must fire first (appears at scene start), so it takes the very first
    # available token. Carve it out of the leading image span to avoid collision.
    first_span = img_spans[0]
    host_phrase = tokens[first_span[0]]
    host_span = [first_span[0], first_span[0] + 1]
    first_span[0] += 1  # shrink the first image span
    if first_span[0] >= first_span[1]:
        img_spans.pop(0)  # span became empty — remove it
    if not img_spans:
        # All tokens went to anchor/host — give images at least the last token
        img_spans = [[max(1, len(tokens) - 1), len(tokens)]]
    print(f"  P0 host phrase: '{host_phrase}'")
    ...
    out_path = Path("assets/tmp") / f"{scene_id}_layout_4_step_2_tmp.json"
