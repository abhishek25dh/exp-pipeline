"""
Layout 31 Step 2 - AI Descriptions for img_left and img_right
==============================================================
Arrow (static local asset) and text_label need no AI generation.
"""
...
def main():
    ...
    step1_path = Path("assets/tmp") / f"{scene_id}_layout_31_step_1_tmp.json"
    ...
    n_active   = step1.get("n_active", 1)

    # Collect only active AI images (arrow and text_label need no description)
    candidates = [
        {"img_id": "img_left",  "phrase": step1.get("img_left_phrase",  "")},
        {"img_id": "img_right", "phrase": step1.get("img_right_phrase", "")},
    ]
    images = [c for c in candidates if c["phrase"]]

    if not images:
        out_path = Path("assets/tmp") / f"{scene_id}_layout_31_step_2_tmp.json"
